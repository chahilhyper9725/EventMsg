...
#define DEVICE00 (uint8_t)0x00
#define DEVICE01 (uint8_t)0x01
#define DEVICE02 (uint8_t)0x02
#define DEVICE03 (uint8_t)0x03
#define DEVICE04 (uint8_t)0x04
#define DEVICE05 (uint8_t)0x05
#define DEVICE06 (uint8_t)0x06
...
void setup()
{
    ...
    espNowDispatcher.on("status", [](const char *data, EventHeader &header)
                        {
    ...
    eventMsg.registerDispatcher("filetransfer",mobileDispatcher.createHeader(DEVICE01, GROUP01),mobileDispatcher.getHandler());
    eventMsg.registerDispatcher("esp_now",espNowDispatcher.createHeader(DEVICE02, GROUP01),espNowDispatcher.getHandler());
    eventMsg.registerDispatcher("ble_control",bleDispatcher.createHeader(DEVICE03, GROUP00),bleDispatcher.getHandler());
...
void loop()
{
    ...
    if(dataflag){
        eventMsg.process(data, data_len);
        dataflag = false;
    }
